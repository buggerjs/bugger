...
const debug = require('debug')('bugger:debugger');
...
// Remove the global flag so we get the proper group matches
const mapFileCommentRegex =
  new RegExp(convert.mapFileCommentRegex.toString().slice(1, -3), 'm');

function getExistingSourceMap(content, url) {
  if (!url) return undefined;

  const mapFileMatch = content.match(mapFileCommentRegex);
  if (mapFileMatch !== null) {
    if (!mapFileMatch[1] && !mapFileMatch[2]) {
      debug('Invalid sourcemap', url, mapFileMatch);
      return undefined;
    }
    return Url.resolve(url, mapFileMatch[1] || mapFileMatch[2]);
  }
  return undefined; // no source map
}

const ScopeTypes = [ 'global', 'local', 'with', 'closure', 'catch', 'block', 'script' ];
function exportScope(objectGroup, scope) {
  return {
...
function exportFrame(raw) {
  ...
  const scopes = raw.allScopes()
    .map(_.partial(exportScope, '@break'));
...
function extractBreakpointId(breakpoint) {
  const scriptBreakpoint = breakpoint.script_break_point();
  if (scriptBreakpoint) {
    return '' + scriptBreakpoint.number();
  }
...
class DebuggerAgent extends BaseAgent {
  ...
  _onException(event, state) {
    this._state = state;
    this.emit('paused', {
      callFrames: this._buildCallStack(),
      reason: 'exception',
      data: ObjectGroup.add('@break', event.exception()),
    });
  }

  _cleanUpContinueBreakpoint() {
    if (this._continueBreakpoint !== null) {
      this._continueBreakpoint.disable();
      this._continueBreakpoint.clear();
      this._continueBreakpoint = null;
    }
  }

  _onBreak(event, state) {
    const breaks = event.breakPointsHit();

    this._state = state;
    this._cleanUpContinueBreakpoint();
    this.emit('paused', {
      callFrames: this._buildCallStack(),
      hitBreakpoints: (breaks && breaks.map(extractBreakpointId)) || undefined,
      reason: 'other',
    });
  }
  ...
  _registerScript(raw) {
    const context = null;
    const url = UrlMapper.scriptUrlFromName(raw.name());
    const script = {
      scriptId: '' + raw.id(),
      url: url,
      sourceMapURL: getExistingSourceMap(raw.source(), url),
  ...
  setBreakpointsActive(params) {
    for (const breakpoint of Debug.scriptBreakPoints()) {
      if (params.active) {
        breakpoint.enable();
      } else {
        breakpoint.disable();
  ...
  continueToLocation(params) {
    ...
    const breakId = Debug.setScriptBreakPointById(+location.scriptId,
      location.lineNumber, location.columnNumber);
    this._continueBreakpoint = Debug.findBreakPoint(breakId);
    this.resume();
  }
   ...
   */
  stepOver() {
    this._state.prepareStep(Debug.StepAction.StepNext, 1);
    this.resume();
  }
   ...
   */
  stepInto() {
    this._state.prepareStep(Debug.StepAction.StepIn, 1);
    this.resume();
  }
   ...
   */
  stepOut() {
    this._state.prepareStep(Debug.StepAction.StepOut, 1);
    this.resume();
  }
  ...
  getFunctionDetails(params) {
    const parsed = ObjectGroup.parseAndGet(params.functionId);
    const mirror = Debug.MakeMirror(parsed.value);
    const sourceLocation = mirror.sourceLocation();

    const scopes = new Array(mirror.scopeCount());
    for (let idx = 0; idx < scopes.length; ++idx) {
      scopes[idx] = exportScope(parsed.objectGroup, mirror.scope(idx));
    }
    ...
    return {
      details: {
        ...
        functionName: mirror.name(),
        isGenerator: mirror.type() === 'generator',
        scopeChain: scopes,
      },
    };
  }
  ...
  evaluateOnCallFrame(params) {
    const frame = this._state.frame(+params.callFrameId);

    const expression = params.includeCommandLineAPI ?
      `with (__buggerCLI) { (${params.expression}) }` : params.expression;

    let mirror;
    let error;
    try {
      mirror = frame.evaluate(expression,
    ...
    const result = mirror.value();
    if (params.objectGroup === 'console') {
      cliAPI.setLastEval(result);
    }

    return {
      result: ObjectGroup.add(params.objectGroup, result, !!params.generatePreview),
      wasThrown: false,
    };
  }
