...
const debug = require('debug')('bugger:network');
...
class NetworkAgent extends BaseAgent {
  constructor() {
    ...
    tracker.EVENTS.forEach(name => {
      tracker.on(name, payload => {
        debug('forwarding', name);
  ...
  getResponseBody(params) {
    ...
    return tracker.bodyMap[requestId];
  }
   ...
   *
   * @param {CertificateId} certificateId ID of the certificate to get details for.
   *
   * @returns {CertificateDetails} result Certificate details.
   */
...
module.exports = new NetworkAgent();
