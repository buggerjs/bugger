...
class RuntimeAgent extends BaseAgent {
  ...
  callFunctionOn(params) {
    ...
    if (returnByValue) {
      ...
      return {
        result: ObjectGroup.add(target.objectGroup, result),
        wasThrown: false,
      };
    }
  }
  ...
  getProperties(params) {
    ...
    function gatherProperties(mirror, isOwn) {
      if (mirror.type() === 'null') return;

      mirror.properties().forEach(prop => {
        if (params.accessorPropertiesOnly && !(prop.hasGetter() || prop.hasSetter())) {
          return;
        }
        result.push({
          name: prop.name(),
          configurable: prop.canDelete(), // ?
          enumerable: prop.isEnum(),
          writable: !prop.isReadOnly(),
          get: ObjectGroup.add(parsed.objectGroup, prop.getter().value()),
          set: ObjectGroup.add(parsed.objectGroup, prop.setter().value()),
          value: ObjectGroup.add(parsed.objectGroup, prop.value().value()),
          isOwn: isOwn,
        });
      });
      ...
      if (!params.ownProperties) {
        gatherProperties(mirror.protoObject(), false);
      }
    }
    /* eslint new-cap: 0 */
    gatherProperties(Debug.MakeMirror(parsed.value), true);
