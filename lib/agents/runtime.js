...
var _, debug, expressionToHandle, handleToExpression, languageMode, LOOKUP_TIMEOUT, makePropertyHandle, prepareEvaluation, refToObject, RuntimeAgent, throwErr, toJSONValue, wrapperObject;
...
makePropertyHandle = cache$.makePropertyHandle;
LOOKUP_TIMEOUT = 2500;
languageMode = 'js';
module.exports = RuntimeAgent = {
  evaluate: function (options, cb, channel) {
    ...
    return debug.request('evaluate', { 'arguments': args }, function (result) {
      ...
      if (result.success) {
        ...
        return throwErr(cb, result.message);
      }
    });
  },
  callFunctionOn: function (options, cb) {
    var args, cache$1, cache$2, disable_break, expression, frame, functionDeclaration, objectId, ref, returnByValue, scope;
    cache$1 = options;
  ...
  getProperties: function (param$, cb) {
    var args, cache$1, cache$2, disable_break, expression, frame, functionDeclaration, getOwnProperties, handle, handles, objectId, ownProperties, ref, scope, timeout;
    {
      cache$1 = param$;
      objectId = cache$1.objectId;
