...
function loadAgentClasses() {
  ...
  for (const kebabDomain of fs.readdirSync(__dirname + '/agents')) {
    ...
    byDomain[kebabDomain] = require('./agents/' + kebabDomain);
  }
  return byDomain;
}
...
class Agents extends events.EventEmitter {
  ...
  _linkEvents(domain, agent) {
    ...
    const pascalCaseDomain = agent.constructor.name.replace(/Agent$/, '');
    agent.on('_event', (eventName, params) => {
      this._sendJSON({ method: `${pascalCaseDomain}.${eventName}`, params });
    });
  }
  ...
  dispatch(domainMethod, params) {
    ...
    if (!agent) {
      throw new Error('Could not find agent: ' + parts[0]);
    }
  ...
  get(domain) {
    return this._byDomain[_.kebabCase(domain)];
  }
}
