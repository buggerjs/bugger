...
var callbackWrapper, debugClient, debugParser, defaults, EventEmitter, mapFrame, mapScope, mapScript, mapValue, NEXT_SEQ, parallel;
...
module.exports = debugClient = function (debugConnection) {
  ...
  registerRequest('backtrace', function (refs) {
    return function (param$, cb) {
      ...
      tasks = frames.map(function (param$1) {
        var index;
        index = param$1.index;
        return function (cb) {
          return client.scopes({ frameNumber: index }, function (err, res) {
            var scope;
            if (null != err)
              return cb(err);
            for (var i$ = 0, length$ = res.scopes.length; i$ < length$; ++i$) {
              scope = res.scopes[i$];
              refs['scope:' + index + ':' + scope.index] = scope;
            }
            return cb();
          });
        };
