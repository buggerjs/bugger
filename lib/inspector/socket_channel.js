...
var _, agents, debug, detectLogLevel, entryScript, fs, knownLogLevels, knownLogLevelsToRegExp, level, names, path, regexp, SocketChannel, socketChannel, wrapperObject;
fs = require('fs');
path = require('path');
...
wrapperObject = require('../wrap_and_map').wrapperObject;
knownLogLevels = {
  debug: [
    'debug',
    'silly'
...
for (level in knownLogLevels) {
  names = knownLogLevels[level];
  regexp = new RegExp(names.map(function (name) {
    return '^' + name + ':|\\[' + name + '\\]';
  }).join('|'), 'ig');
  knownLogLevelsToRegExp[level] = regexp;
}
detectLogLevel = function (data, defaultLevel) {
  var str;
  str = data.toString();
  for (level in knownLogLevelsToRegExp) {
    regexp = knownLogLevelsToRegExp[level];
    if (regexp.test(str))
      return level;
  }
  return defaultLevel;
...
SocketChannel = function () {
  function SocketChannel(param$) {
    var cache$, instance$, this$, this$1, this$2, this$3, this$4;
    instance$ = this;
    this.sendParsedScripts = function (c) {
      return SocketChannel.prototype.sendParsedScripts.apply(instance$, arguments);
    };
    this.onAfterCompile = function (c) {
      return SocketChannel.prototype.onAfterCompile.apply(instance$, arguments);
    };
    ...
    this.onResumed = function () {
      return SocketChannel.prototype.onResumed.apply(instance$, arguments);
    };
    {
      cache$ = param$;
    ...
    debug.on('resumed', this.onResumed);
    debug.on('afterCompile', this.onAfterCompile);
    entryScript.proc.stdout.on('data', (this$ = this, function (data) {
      return this$.console(data, detectLogLevel(data, 'log'));
    }));
    entryScript.proc.stderr.on('data', (this$1 = this, function (data) {
      return this$1.console(data, detectLogLevel(data, 'debug'));
    }));
    entryScript.on('message', (this$2 = this, function (message) {
      return this$2.dispatchEvent(message.method, _.omit(message, 'method'));
    }));
    this.socketConnection.on('message', (this$3 = this, function (data) {
      return this$3.handleRequest(JSON.parse(data.utf8Data));
    }));
    this.socketConnection.on('disconnect', (this$4 = this, function () {
      return this$4.socketConnection = null;
    }));
    this.syncBrowser();
  }
  ...
  SocketChannel.prototype.handleRequest = function (msg) {
    ...
    if (msg.method) {
      ...
      return console.error('[bugger] Unknown message from frontend:', msg);
    }
  };
  SocketChannel.prototype.sendResponse = function (seq, err, data) {
    ...
    if (this.socketConnection) {
      ...
      return console.error('[bugger] Could not send response: ', req, data);
    }
  };
  SocketChannel.prototype.dispatchEvent = function (method, params) {
    ...
    if (this.socketConnection) {
      ...
      return console.error('[bugger] Could not dispatch event: ', method, params);
    }
  };
  ...
  SocketChannel.prototype.onResumed = function () {
    return this.dispatchEvent('Debugger.resumed');
  };
  ...
  SocketChannel.prototype.onAfterCompile = function (afterCompileEvent) {
    var args;
    args = {
      'arguments': {
        includeSource: true,
    ...
    return debug.request('scripts', args, this.sendParsedScripts);
  };
