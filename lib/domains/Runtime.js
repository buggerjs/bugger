...
module.exports = function (param$) {
  ...
  Runtime.getProperties = function (param$1, cb) {
    var cache$, cache$1, evaluate, expression, lookup, objectGroup, objectId, objIdParts, options, ownProperties, params, patchedProperties, propertyPath, propName, subId;
    {
      cache$ = param$1;
      objectId = cache$.objectId;
    ...
    if (objectId.substr(0, 6) === 'scope:' || /^\d+$/.test(objectId)) {
      ...
      if (null != objectGroup && null != subId) {
        cache$1 = debugClient.commands;
        evaluate = cache$1.evaluate;
        lookup = cache$1.lookup;
        options = {
          returnByValue: true,
          doNotPauseOnExceptionsAndMuteConsole: true
        };
        expression = 'root.__bugger__[' + JSON.stringify(objectGroup) + '][' + JSON.stringify(subId) + ']';
        propertyPath = objIdParts.length > 0 ? JSON.parse(objIdParts.join(':')) : [];
        for (var i$ = 0, length$ = propertyPath.length; i$ < length$; ++i$) {
          propName = propertyPath[i$];
          expression += '[' + JSON.stringify(propName) + ']';
        }
        patchedProperties = function (properties) {
          var baseObjectId, propObjectId;
          baseObjectId = '' + objectGroup + ':' + subId + ':';
          propObjectId = function (name) {
            return baseObjectId + JSON.stringify(propertyPath.concat([name]));
          };
          properties.forEach(function (property) {
            if (null != (null != property.value ? property.value.objectId : void 0))
              property.value.objectId = propObjectId(property.name);
            return property;
          });
          return cb(null, { result: properties });
        };
        return evaluate(options)(expression, function (err, remoteObject) {
          if (null != err)
            return cb(err);
          if (null != remoteObject.properties)
          ...
          return lookup(options)(remoteObject.objectId, function (err, objectDescriptor) {
            if (null != err)
              return cb(err);
            return cb(null, { result: patchedProperties(objectDescriptor.properties) });
          });
        });
      }
    }
